...
#' Implements UNblocked Gibbs sampler where step 2 of Algorithm 1 is changed to full conditional
#'
...
#'
#' @examples
LBBer_Matern_unblocked <- function(z, x, a, b, range_grid, smoothness = 1.5, Dxx =NULL,
                             nburn = 100, nsave = 1000, nthin = 1, nparticle = 5, adapt = T, verbose=TRUE){
  ...
  for(imcmc in 1:(nburn + nsave*nthin)){
    ...
    if(indepMH){
      ...
      # unblocked
      logprob_new = -n/2*log(lambda_new) - as.numeric(t(eta - rep(0.5*lambda_new*(a-b), n))%*%Rxxinv%*%(eta - rep(0.5*lambda_new*(a-b), n)))/(2*lambda_new)
      
      #logprob_old = mvnfast::dmvn((z-0.5)/omega,
      #                            mu = rep(0.5*lambda*(a-b), n),
      #                            sigma = diag(1/omega)+lambda*Rxx, log = T)
      # unblocked
      logprob_old = -n/2*log(lambda) - as.numeric(t(eta - rep(0.5*lambda*(a-b), n))%*%Rxxinv%*%(eta - rep(0.5*lambda*(a-b), n)))/(2*lambda)
      ...
      for(iparticle in 1:(nparticle+1)){
        ...
        #                                       log = T)
        # unblocked
        logweights[iparticle] = -n/2*log(lambda_particles[iparticle]) - as.numeric(t(eta - rep(0.5*lambda_particles[iparticle]*(a-b), n))%*%Rxxinv%*%(eta - rep(0.5*lambda_particles[iparticle]*(a-b), n)))/(2*lambda_particles[iparticle])
      }
